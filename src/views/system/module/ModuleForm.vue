  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="模块code" prop="code">
        <el-input v-model="formData.code" placeholder="请输入模块code" />
      </el-form-item>
...
import { ModuleApi, ModuleVO } from '@/api/system/module'
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ModuleApi.getModule(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as ModuleVO
    if (formType.value === 'create') {
      await ModuleApi.createModule(data)
      message.success(t('common.createSuccess'))
    } else {
      await ModuleApi.updateModule(data)
